...
export default function CodeOracle() {
  ...
  // Функция для синхронизации данных из FileTree (если там изменят владельца/репозиторий)
  const handleRepoInfoChange = (owner: string, repo: string) => {
    setRepoInfo({ owner, repo });
    localStorage.setItem("oracle_owner", owner);
    localStorage.setItem("oracle_repo", repo);
    // Автоматически загружаем дерево
    loadRepoTree(owner, repo);
  };

  const loadRepoTree = async (owner?: string, repo?: string) => {
    const finalOwner = owner || repoInfo.owner;
    const finalRepo = repo || repoInfo.repo;
    if (!finalOwner || !finalRepo) return;
    try {
      const res = await fetch(`/api/github/tree?owner=${finalOwner}&repo=${finalRepo}`);
  ...
  const handleFileClick = async (path: string) => {
    console.log("Клик по файлу, путь:", path); // ОТЛАДКА
        ...
        <div className="h-12 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-4">
          ...
          {!isImage && (
            <button onClick={handlePush} disabled={isPushing} className="bg-emerald-900/30 text-emerald-400 text-[10px] px-4 py-1 rounded border border-emerald-900/50 font-bold hover:bg-emerald-800/50 transition-all">
              {isPushing ? "ПРОЦЕСС..." : "МАТЕРИАЛИЗОВАТЬ (PUSH)"}
            </button>
          )}
        </div>
