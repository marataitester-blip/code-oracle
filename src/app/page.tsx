...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Пытаемся найти файл, соответствующий текущему маршруту
  ...
  useEffect(() => {
    ...
    setAdvice(`💡 Фокус: ${filename}. Если ищете картинку, скажите Оракулу, какой текст написан рядом с ней.`);
  }, [activeFile, isConnected]);

  // Авто-сканер страниц (Усиленный бульдозер)
  useEffect(() => {
    if (files.length === 0) return;
    const generated: RouteEntry[] = [];
    files.forEach(f => {
      // Ищем все файлы page.tsx, page.jsx, page.js где угодно в проекте
      if (f.path.match(/(^|\/)page\.(tsx|jsx|js)$/)) {
        // Вырезаем src/app/, app/ и само имя файла, чтобы получить чистый маршрут
        let route = f.path.replace(/^(src\/)?app\//, '').replace(/\/?page\.[tj]sx?$/, '');
        ...
        if (route === '') {
            ...
            // Защита от дублей
            if (!generated.find(r => r.path === route)) {
  ...
  const handleSendMessage = async () => {
    ...
    // ЖЕСТКАЯ ДИРЕКТИВА: Внедрена инструкция по слепоте Оракула
    const enforcedPrompt = `[АРХИТЕКТУРА ПРОЕКТА]:\n${architectureMap}\n\n[КОНТЕКСТ]:\nТекущий экран (Визор): /${currentRoute}\nОткрытый файл (Фокус): ${activeFile || 'Не выбран'}\n\n[ЗАПРОС ИНЖЕНЕРА]:\n${text}\n\n[СИСТЕМНАЯ ДИРЕКТИВА ИНЖЕНЕРА]:\n1. ВОЗВРАТ КОДА: Ты ОБЯЗАН вернуть ПОЛНЫЙ, рабочий код файла от начала до конца без сокращений (код пойдет напрямую в компилятор Vercel).\n2. СЛЕПОТА ОРАКУЛА: Ты НЕ видишь экран Визора визуально. Если инженер просит изменить "картинку" или "кнопку справа", но не дает контекста, объясни ему, что ты читаешь только код. Попроси инженера назвать ЛЮБОЙ ТЕКСТ, который находится рядом с этим элементом на экране, чтобы ты смог найти его в коде через поиск (Ctrl+F).\n3. НАВИГАЦИЯ ПО ИМПОРТАМ: Если элемент скрыт в дочернем компоненте, изучи секцию 'import' открытого файла, найди компонент в [АРХИТЕКТУРА ПРОЕКТА] и назови точный путь к файлу.`;
