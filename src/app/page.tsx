...
export default function App() {
  ...
  const handleSendMessage = async () => {
    ...
    // ЖЕСТКАЯ ДИРЕКТИВА: Принуждаем ИИ выдавать полный код, скармливаем ему карту и включаем РЕЖИМ СЫЩИКА
    const enforcedPrompt = `[АРХИТЕКТУРА ПРОЕКТА]:\n${architectureMap}\n\n[КОНТЕКСТ]:\nТекущий экран (Визор): /${currentRoute}\nОткрытый файл (Фокус): ${activeFile || 'Не выбран'}\n\n[ЗАПРОС ИНЖЕНЕРА]:\n${text}\n\n[СИСТЕМНАЯ ДИРЕКТИВА ИНЖЕНЕРА]:\n1. ВОЗВРАТ КОДА: Ты ОБЯЗАН вернуть ПОЛНЫЙ, рабочий код файла от начала до конца без сокращений (код пойдет напрямую в компилятор Vercel).\n2. РЕЖИМ СЫЩИКА (Связь Визора и Репозитория): Если инженер спрашивает, где лежит визуальный элемент с экрана Визора, сопоставь текущий экран, открытый файл и архитектуру. Если элемент скрыт в дочернем компоненте (например, картинка внутри <Card />), изучи секцию 'import' открытого файла, найди этот компонент в [АРХИТЕКТУРА ПРОЕКТА] и назови инженеру точный путь к файлу для следующего шага.`;
